 const chapters = [
		{
			id: 1,
			title: "CHAPTER 1: Introduction",
			topics: [
				"The Nature and Uses of History",
				"Sources and Methods of Historical Study",
				"Historiography of Ethiopia and the Horn",
				"The Geographical Context",
			],
		},
		{
			id: 2,
			title: "CHAPTER 2: Peoples and Cultures in Ethiopia and the Horn",
			topics: [
				"Human Evolution ",
				"Neolithic Revolution ",
				"Neolithic Revolution ",
				"Religion and Religious Processes",
			],
		},
		{
			id: 3,
			title: "CHAPTER 3: Politics, Economy, and Society to the End of the 13th Century",
			topics: [
				"Emergence of States",
				"Ancient States",
				"External Contacts ",
				"The Geographical Context",
				"Socio-cultural Achievements "
			],
		},
		{
			id: 4,
			title: "CHAPTER 4: Politics, Economy, and Society to the End of the 13th Century",
			topics: [
				"The “Restorations” of the “Solomonic”” Dynasty ",
				"Power Struggle, Consolidation, Territorial Expansion and Religious Processes ",
				"Political and Socio-economic Dynamics in Muslim Sultanates ",
				"Rivalry between the Christian Kingdom and the Muslim Sultanates ",
				"External Relations  "
			],
		},
		{
			id: 5,
			title: "CHAPTER 5: Politics, Economy, and Society to the End of the 13th Century",
			topics: [
				"Conflict between the Christian Kingdom and the Sultanate of Adal",
				"Foreign Intervention and Religious Controversies",
				"Population Movements ",
				"Interaction and Integration across Ethnic and Religious Diversities ",
				"Peoples and States in Eastern, Central, Southern and Western Regions",
				"The Gondarine Period and the Zmene-Mesafint"
			],
		},
		{
			id: 6,
			title: "CHAPTER 6: Internal Development and External Relations of Ethiopia and the Horn 1800-1941",
			topics: [
				"The Nature of Interactions among Peoples and States of Ethiopia and the Horn",
				"The Making of Modern Ethiopian State ",
				"Modernization Attempts  ",
				"Socio-economic Developments ",
				"External Relations"
			],
		},
		{
			id: 7,
			title: "CHAPTER 7: Internal Developments and External Relations (1941-1995)",
			topics: [
				"Post-1941 Imperial Period ",
				"The Derg Regime (1974-1991) ",
			],
		},
	];

	export default chapters

